...
xtest_collection<-cbind(subject_test,ytest,subset(xtest,select=columns_means_std))
xtrain_collection<-cbind(subject_train,ytrain,subset(xtrain,select=columns_means_std))
...
tidy<-aggregate(xymerged [,3:ncol(Xy)],list(Subject= xymerged$subjects, Activity= xymerged $labels), mean)
